...
const { userOneId, userOne, setupDatabase, userTwoId, userTwo, taskOneId, taskTwo } = require('./fixtures/db')
...
test('Should not let userTwo delete userOne tasks', async () => {
    const response = await request(app).delete(`/tasks/${taskOneId}`).set('Authorization', `Bearer ${userTwo.tokens[0].token}`).send().expect(404)
